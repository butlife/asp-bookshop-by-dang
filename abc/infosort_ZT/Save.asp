...
<%
	...
	Function adminsave()
		...
		conn.BeginTrans	'开始
		strSql = "Select * From infosort_ZT Where SortName = '" & strSortName & "' and sortid <> " & lngSortId
		...
		End If
		If (sType = "add") Then
			strSql = "Select * From infosort_ZT Where 1 = 2"
			If rsSort.State = 1 Then rsSort.Close
			rsSort.Open strSql, conn, 2, 3
			rsSort.AddNew
		ElseIf (sType = "modify") then
			strSql = "Select * From infosort_ZT Where sortid = " & lngSortId
			If rsSort.State = 1 Then rsSort.Close
			rsSort.Open strSql, conn, 2, 3
		End If
