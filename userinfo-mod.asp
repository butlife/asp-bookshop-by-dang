  ...
  <body style="padding-top:50px;">
        ...
        <div class="row show-grid">
          <div class="col-xs-12">
            <div class="input-group input-group-lg">
              ...
              <input value="<%=strUserName%>" type="text" class="form-control" placeholder="会员姓名、昵称" name="username" id="username" aria-describedby="sizing-username"  required autofocus>
            </div>
          </div>
        </div>
        <div class="row show-grid">
          <div class="col-xs-12">
            <div class="input-group input-group-lg">
              <span class="input-group-addon" id="sizing-userSex">性别</span>
        ...
        <div class="row show-grid">
          <div class="col-xs-12">
            <div class="input-group input-group-lg" style="height:70px;">
              <span class="input-group-addon" id="sizing-useradd">地址</span>
                <textarea class="form-control" rows="3" name="useradd" placeholder="您的收货地址" id="useradd" style="height:70px;" required autofocus><%=struseradd%></textarea>
            </div>
          </div>
        </div>
        <div class="row show-grid">
          <div class="col-xs-12">
            <div class="input-group input-group-lg" style="height:70px;">
              <span class="input-group-addon" id="sizing-hobby">爱好</span>
                <textarea class="form-control" rows="3" name="hobby" id="hobby" placeholder="您的兴趣爱好" style="height:70px;"><%=strhobby%></textarea>
            </div>
          </div>
        </div>
        <hr>
    ...
    <script>
		$("#form-usermod").submit( function(){
			$.ajax({
			url:  "service/userinfo-mod.asp", 
			data:$("#form-usermod").serialize(), 
			dataType:'json', 
			type:'post', 
			success:function(data){
				if (data.state == 0) {alert("修改资料成功");location.href = "main.asp";}
