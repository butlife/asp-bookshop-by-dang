  ...
  <body style="padding-top:50px;">
    ...
    <div class="container">
        ...
        <div class="row show-grid">
          <div class="col-xs-12 text-center"><%=strUserName%></div>
        </div>
        <hr>
        <div class="row show-grid">
        ...
        <hr>
        <div class="row show-grid">
          <div class="col-xs-12">有效日期：<%=strexpdate_s%> - <%=strexpdate_e%></div>
        </div>
        <hr>
        <div class="row show-grid">
          <div class="col-xs-12">送书地址：<%=struserAdd%></div>
        </div>
        <hr>
        <div class="row show-grid">
          <div class="col-xs-12">单次最大借书数量：<%=lnguseCounts%>本</div>
        </div>
        <hr>
        <div class="row show-grid">
          <div class="col-xs-12">最大借书次数：已用<%=lngmaxuseCounts-lngmaxuseCountsTemp%>次/剩余<%=lngmaxuseCountsTemp%>次/共计<%=lngmaxuseCounts%>次</div>
        </div>
    ...
    <script>
		$(function() { 
			...
			$("#service-tel").click(function(){location.href = "tel:013937277704";});
		});
	</script>
  </body>
</html>